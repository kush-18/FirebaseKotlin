...
class InsertionActivity : AppCompatActivity() {
    ...
    private fun saveEmployeeData() {
        ...
        if(empAge.isEmpty()){
            etEmpAge.error = "Please Enter the Employee Age"

        }
        if(empName.isEmpty()){

            etEmpName.error = "Please Enter the Employee Name"

        }

        if(empSalary.isEmpty()){

            etEmpSalary.error = "Please Enter the Employee Salary"
        ...
        dbRef.child(empId).setValue(employee).addOnCompleteListener {
            Toast.makeText(this, "Data is succesfully inserted", Toast.LENGTH_LONG).show()
